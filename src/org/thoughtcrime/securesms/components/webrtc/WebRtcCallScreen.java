...
public class WebRtcCallScreen extends FrameLayout implements RecipientModifiedListener {
    ...
    private boolean localPreview = false;
    ...
    public void setUntrustedIdentity(Recipient personInfo, IdentityKey untrustedIdentity) {
        ...
        this.endCallButton.setVisibility(View.GONE);
    }
    ...
    private void setConnected(SurfaceViewRenderer localRenderer,
        ...
        if (localRenderLayout.getChildCount() == 0 && remoteRenderLayout.getChildCount() == 0) {
            ...
            localRenderLayout.setPosition(7, 65, 25, 25);
            remoteRenderLayout.setPosition(0, 0, 100, 100);
            localRenderLayout.setSquare(false);
            ...
            localRenderer.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (!localPreview) {
                        localPreview = true;
                        localRenderLayout.removeView(localRenderer);
                        remoteRenderLayout.removeView(remoteRenderer);
                        localRenderer.setZOrderMediaOverlay(false);

                        localRenderLayout.addView(remoteRenderer);
                        remoteRenderLayout.addView(localRenderer);
            ...
            remoteRenderer.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (localPreview) {
                        localPreview = false;
                        localRenderLayout.removeView(remoteRenderer);
                        remoteRenderLayout.removeView(localRenderer);
    ...
    private void setMinimized(boolean minimized) {
        if (minimized) {
            ViewCompat.animate(callHeader).translationY(-1 * callHeader.getHeight());
            ViewCompat.animate(controls).translationY(controls.getHeight() + ViewUtil.dpToPx(getContext(), 80));


            ViewCompat.animate(endCallButton).translationY(endCallButton.getHeight() + ViewUtil.dpToPx(getContext(), 80));
            ...
            ViewCompat.animate(controls).translationY(0);
